...
$(document).ready(function(){
    ...
    $("#sign_btn").click(function(){
        window.location.href = (supporthttps==1 ? 'https://' : 'http://') + 'login.51job.com/register.php?lang='+sLang+'&from_domain=i&source=&url='+encodeURIComponent(window.cfg.domain.www+"/default.php");
    });
    
    $("#create_resume").click(function(){
        window.location.href = window.cfg.domain.i+"/resume/tri_resume.php";
    });
    
    //首页导航浮层
    var showguide = $("#showguide").val();
    if(showguide == '1'){
        var html = '<style>.pop{width:100%;height:100%;background-color:#000;opacity:.5}.home_guide{position:relative;width:1002px;margin:0 auto;margin-top:45px}.home_guide .close{position:absolute;width:30px;height:30px;right:0;background:url(//img01.51jobcdn.com/im/2016/temp/icon_close.png) center no-repeat;cursor:pointer}.home_guide .bg{width:1002px;height:400px;margin-bottom:30px;background:url(//img01.51jobcdn.com/im/2016/temp/IMG_sxl.png) center no-repeat}.home_guide .btn{width:182px;float:none;height:60px;margin:0 auto;background:url(//img01.51jobcdn.com/im/2016/temp/butte_ljty.png) center no-repeat;cursor:pointer}.home_guide.h2 .bg{width:1002px;height:384px;margin-bottom:0;background:url(//img01.51jobcdn.com/im/2016/temp/IMG_dlh.png) center no-repeat}.home_guide.h2 .btn{position:absolute;width:101px;height:44px;left:505px;background:url(//img01.51jobcdn.com/im/2016/temp/butten_zdl.png) center no-repeat;cursor:pointer}</style><div class="pop"></div><div class="home_guide"><div class="close"></div><div class="bg"></div><div class="btn" onclick="$(\'.close\').click();"></div></div>';
    ...
    if(showguide == '1'){
        var html = '<style>.pop{width:100%;height:100%;background-color:#000;opacity:.5}.home_guide{position:relative;width:1002px;margin:0 auto;margin-top:45px}.home_guide .close{position:absolute;width:30px;height:30px;right:0;background:url(//img01.51jobcdn.com/im/2016/temp/icon_close.png) center no-repeat;cursor:pointer}.home_guide .bg{width:1002px;height:400px;margin-bottom:30px;background:url(//img01.51jobcdn.com/im/2016/temp/IMG_sxl.png) center no-repeat}.home_guide .btn{width:182px;height:60px;margin:0 auto;background:url(//img01.51jobcdn.com/im/2016/temp/butte_ljty.png) center no-repeat;cursor:pointer}.home_guide.h2 .close{top:-160px}.home_guide.h2 .bg{width:1002px;height:384px;margin-bottom:0;background:url(//img01.51jobcdn.com/im/2016/temp/IMG_dlh.png) center no-repeat}.home_guide.h2 .btn{position:absolute;width:101px;height:44px;top:40px;left:505px;background:url(//img01.51jobcdn.com/im/2016/temp/butten_zdl.png) center no-repeat;cursor:pointer}</style><div class="pop"></div><div class="home_guide h2"><div class="close"></div><div class="bg"></div><div class="btn" onclick="$(\'.close\').click();"></div></div>';
        var oLayerParams = {'layer_close_class': 'close'};
        oLayerSettings = jQuery.FLayer.init(oLayerParams); 
        jQuery.FLayer.setContent(oLayerSettings,html);
        jQuery.FLayer.open(oLayerSettings);
    ...
    if(islogin==1){
        //首页个人名片信息
        var url = window.cfg.domain.i + '/userset/ajax/index_cv.php?jsoncallback=?';
        var lang = sLang;   
        var flag = 0;
        ...
        $.getJSON(url,{lang:lang},function(data){
            if(data.resumeid != ""){
                ...
                if(data.resumetype == '9'){
                    var resume_url = window.cfg.domain.i + "/resume/paste_resume.php?lang="+lang+'&resumeid='+data.resumeid+"&"+(Math.round(Math.random()*10));
                }else{
                    var resume_url = window.cfg.domain.i + "/resume/standard_resume.php?lang="+lang+'&resumeid='+data.resumeid+"&"+(Math.round(Math.random()*10));
                }
                ...
                $("#refreshresume").click(function(){
                    if(!$(this).hasClass("unclick")){
                        Refreshresume(data.resumeid,lang,window.cfg.domain.i + '/resume');
                    }
                });
            }else{
                $("#hasresume").hide();
...
function load(){   
    ...
    var aNeedV = ['020111', '010500', '030010', '030011', '030012', '020100', '020101'];
    var loginerr = ['030000', '030010', '030011'];
    if(doc != ""){
        var data = eval('('+doc+')');
        var loginname = $("#loginname").val();
        var userpassword = $("#password").val();
        if(data.result=='0'){ //登录失败
            if(data.error_code == '010500')
            {
                $("#p_err").html(langs['net_err']).show();
                var objDate = new Date();
                var strTime = objDate.getTime();
                var code_url = (supporthttps==1 ? 'https://' : 'http://') + 'login.51job.com/ajax/verifycode.php?from_domain=my&t='+strTime;
            ...
            if($.inArray(data.error_code,loginerr) != -1){
                var str = langs['name_pwd_error'];  
                if(data.error_code == '030010'){
                    changeVerifyCode();
                    str = langs['seek_pwd'];
                }            
            ...
            if($.inArray(data.error_code,aNeedV) != -1){
            ...
            if(data.url != ""){
                ...
                window.location.href = (supporthttps==1 ? 'https' : 'http') +'://www.51job.com'+(sLang == 'c' ? '' : '/default-e.php');
            }
        }   
    } 
}
...
function checkOK(){
    ...
    {
        $("#login_form").attr("action",window.cfg.domain.login + '/ajax/login.php');
    }else{
        var sLang = $("#language").val();    
        var url = (supporthttps==1 ? 'https' : 'http') +'://www.51job.com'+(sLang == 'c' ? '' : '/default-e.php')
        $("#login_form").attr("action",'https://login.51job.com/login.php?url='+url);
        $("#login_form").attr("target",'');
    }
    return true;
}
...
function checkVerifyCode(){
    ...
    var flag = 0;
    $.ajaxSettings.async = false;
    $.getJSON(url,{verifycode:verifycode,from_domain:'my'},function(data){
        if(data.result == '0'){
            flag = 1;           
        }
    });
...
function changeVerifyCode(){
    var objDate = new Date();
    var strTime = objDate.getTime();
    var url = $('#verifyPic_img').attr('src');
    if(url.indexOf('?') > 0)
    {
        url = url.replace(/\?.*/g,'?');
    }else
    ...
    url = url + 'from_domain=my&t=' + strTime;
...
{	
	var request_url = window.cfg.domain.i + '/resume/ajax/refresh_resume.php?'+ Math.random() + '&jsoncallback=?';
